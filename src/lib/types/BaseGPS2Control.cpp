...
void BaseGPS2Control::read(POSLVControl& stream) throw (IOException) {
  ...
  stream >> pad;
  if (pad != 0)
    throw IOException("BaseGPS2Control::read(): wrong pad");
  mChecksum += pad;
  mChecksum += 8996; // for $#
  mChecksum = 65536 - mChecksum;
}

void BaseGPS2Control::write(POSLVControl& stream) const {
